...
A script used to modify the protection of an Azure IaaS VM backup of a specific VM to only backup the OS disk.
...
Modify protection to only backup the OS disk

.NOTES

Filename:       Modify-Azure-VM-Backup-to-OS-disk-only.ps1
Created:        15/03/2023
...
.\Modify-Azure-VM-Backup-to-OS-disk-only.ps1 <"your VM name here">

-> Modify-Azure-VM-Backup-to-OS-disk-only.ps1 swpdc004
...
Enable-AzRecoveryServicesBackupProtection -Item $backupItem -ExcludeAllDataDisks -VaultId $vault.ID -Policy $policy | Out-Null

Write-Host ($writeEmptyLine + "# Protection VM $vmName set to backup only the OS disk" + $writeSeperatorSpaces + $currentTime)`
